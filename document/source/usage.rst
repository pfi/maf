...
``--simple_param``
   各ノード上の出力として、パラメータの組み合わせの代わりに一つの数値が使われます。この数値は、パラメータの一つの組み合わせに対応する ``id`` となっています。この ``id`` とパラメータの対応は、./build/.maf_id_table.tsvというファイルを参照することにより得ることができます。一つのノードに用いられるパラメータの数が大きい場合には、この機能によりシンプルなグラフを描画することができます。
